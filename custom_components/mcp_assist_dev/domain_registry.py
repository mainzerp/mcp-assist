...
DOMAIN_REGISTRY = {
    ...
    "light": {
        ...
        "parameters": {
            "turn_on": {
                "optional": ["transition", "brightness", "brightness_pct", "brightness_step", "brightness_step_pct",
                           "rgb_color", "rgbw_color", "rgbww_color", "color_name", "hs_color", "xy_color",
                           "color_temp", "color_temp_kelvin", "white", "profile", "flash", "effect"]
            },
            "turn_off": {
                "optional": ["transition", "flash"]
            },
            "toggle": {
                "optional": ["transition", "brightness", "brightness_pct", "rgb_color", "color_temp", "effect"]
            }
        },
    ...
    "cover": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_ESSENTIAL,
        "services": ["open_cover", "close_cover", "stop_cover", "toggle", "set_cover_position",
    ...
    "climate": {
        ...
        "services": ["set_temperature", "set_hvac_mode", "set_preset_mode", "set_fan_mode",
                    "set_humidity", "set_swing_mode", "set_swing_horizontal_mode", "turn_on", "turn_off", "toggle"],
        "parameters": {
            "set_temperature": {
                "optional": ["temperature", "target_temp_high", "target_temp_low", "hvac_mode"]
            },
            "set_hvac_mode": {
                "required": ["hvac_mode"]
            },
    ...
    "media_player": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_COMMON,
        "services": ["turn_on", "turn_off", "toggle", "volume_up", "volume_down", "volume_set", "volume_mute",
                    ...
                    "clear_playlist", "shuffle_set", "repeat_set", "join", "unjoin",
                    "browse_media", "search_media"],
        "parameters": {
            "volume_set": {
                "required": ["volume_level"]
            },
            "volume_mute": {
                "required": ["is_volume_muted"]
            },
            "media_seek": {
            ...
            "browse_media": {
                "optional": ["media_content_type", "media_content_id"]
            },
            "search_media": {
                "required": ["search_query"],
                "optional": ["media_content_type", "media_content_id", "media_filter_classes"]
            }
        },
    ...
    "vacuum": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_COMMON,
        "services": ["turn_on", "turn_off", "toggle", "start", "stop", "pause", "start_pause",
                    "return_to_base", "clean_spot", "locate", "send_command", "set_fan_speed"],
        "parameters": {
            "send_command": {
                "required": ["command"],
                "optional": ["params"]
    ...
    "alarm_control_panel": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_COMMON,
        "services": ["alarm_disarm", "alarm_arm_home", "alarm_arm_away", "alarm_arm_night",
    ...
    "camera": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_COMMON,
        "services": ["turn_on", "turn_off", "enable_motion_detection", "disable_motion_detection",
                    "snapshot", "record", "play_stream"],
        "parameters": {
            "snapshot": {
                "required": ["filename"]
            },
    ...
    "device_tracker": {
        ...
        "parameters": {
            "see": {
                "optional": ["mac", "dev_id", "host_name", "location_name", "gps", "gps_accuracy", "battery"]
            }
        },
    ...
    "input_select": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_STANDARD,
        "services": ["select_option", "select_next", "select_previous", "select_first", "select_last",
                    "set_options", "reload"],
        "parameters": {
            "select_option": {
                "required": ["option"]
    ...
    "group": {
        ...
        "parameters": {
            "set": {
                "required": ["object_id"],
                "optional": ["name", "icon", "entities", "add_entities", "remove_entities", "all"]
            },
            "remove": {
                "required": ["object_id"]
            }
    ...
    "select": {
        ...
        "services": ["select_option", "select_first", "select_last", "select_next", "select_previous"],
        "parameters": {
            "select_option": {
                "required": ["option"]
            },
    ...
    "water_heater": {
        ...
        "services": ["set_temperature", "set_operation_mode", "set_away_mode", "turn_on", "turn_off"],
        "parameters": {
            "set_temperature": {
    ...
    "valve": {
        "type": TYPE_CONTROLLABLE,
        "priority": PRIORITY_SPECIALIZED,
        "services": ["open_valve", "close_valve", "set_valve_position", "stop_valve", "toggle"],
        "parameters": {
            "set_valve_position": {
                "required": ["position"]
            }
...
def get_supported_domains(priority: Optional[int] = None) -> List[str]:
    ...
    return [
        domain for domain, info in DOMAIN_REGISTRY.items()
...
def validate_domain_action(domain: str, action: str) -> Tuple[bool, str]:
    ...
    if not domain_info:
        ...
        if similar:
            return False, f"Domain '{domain}' not supported. Did you mean: {', '.join(similar[:3])}?"
        return False, f"Domain '{domain}' not supported. Use 'list_domains' to see available domains."

    # Check if domain is read-only
    if domain_info["type"] == TYPE_READ_ONLY:
        return False, domain_info.get("error_message", f"Domain '{domain}' is read-only. Use 'get_entity_details' to read values.")
    ...
    if available_services:
        return False, f"Action '{action}' not valid for {domain}. Available: {', '.join(available_services[:5])}"
...
def validate_service_parameters(domain: str, service: str, provided_params: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if missing:
        return False, f"Missing required parameters for {domain}.{service}: {', '.join(missing)}"
